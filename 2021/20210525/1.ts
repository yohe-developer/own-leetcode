...
function minChildArray(arrTa: any, target: number) {
	// 小于一个 判断大小返回
	if (arrTa.length === 1) return arrTa[0] < target ? arrTa[0] : []
	const dp: any = [[arrTa[0]]] // 记录第一个
	for (let i = 1; i < arrTa.length; i++) {
		const local = arrTa[i]
		const lastDep = dp[i - 1] // 获取前一个

		const sum: any = lastDep.reduce((a: number, b: number) => a + b, 0) // 获取当前的和
...
const array: number[] = [0, 1, 3, -1, 4, 6, -2]
